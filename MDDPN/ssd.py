...
# Last modified: 13-12-2023 19:38:15
...
def main():
    ...
    parser_init.add_argument(
        "-p", "--params", action="store", type=str, help="Obtain simulation parameters from command-line"
    )
    parser_init.add_argument(
        "-rm",
        "--restart_mode",
        choices=["one", "two", "multiple"],
        help="Specify two-filed restarts instead of restart.*",
    )
    ...
    parser_run.add_argument(
        "--test", action="store_true", help="Whether actually run LAMMPS or not. Test purposes only"
    )
    ...
    parser_gen_conf = sub_parsers.add_parser(
        "genconf", help="Generate config file (all possible options with default values)"
    )
